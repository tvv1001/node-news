...
- Backend: Node.js ESM with Express 4 (Unified API and Context Service)
...
- Preserve the API contract between `src/api.ts` and the Express routes.
...
- `src/app/page.tsx` - main Next.js page; renders the React app
- `src/app/layout.tsx` - shared Next.js HTML shell
- `src/components/` - React UI components such as `App.tsx`, `SearchForm.tsx`, and `ResultsList.tsx`
- `src/api.ts` - frontend HTTP client and request helpers for `/api/*`
- `server/index.js` - Unified Express app bootstrap, middleware, and route registration
...
- Treat `src/api.ts` as the canonical client wrapper for backend endpoints.
- When changing UI state flows, keep loading, error, and results handling centralized in `src/components/App.tsx`.
